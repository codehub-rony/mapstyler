<template>
  <v-dialog persistent v-model="dialogIsOpen" width="400">
    <v-card>
      <v-card-title class="text-h6 font-weight-light">{{ title }}</v-card-title>
      <v-card-text>
        ...
        <div v-if="filter">
          <div class="mt-4 text-body-2 font-weight-bold pb-1">Filter</div>
          <LayerCondition
        ...
        <div class="d-flex justify-center mt-2">
          <v-btn flat size="x-small" rounded="0" @click="createCondition"
            ><v-icon class="mr-2">mdi-plus</v-icon>condition</v-btn
          >
        </div>
      </v-card-text>
      <v-divider></v-divider>
